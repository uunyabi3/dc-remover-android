...
@Singleton
class DcRepositoryImpl @Inject constructor(
    ...
    override val isLoggedIn: Boolean get() = _userId.isNotEmpty()
    ...
    override suspend fun getGalleries(postType: PostType): Result<List<GalleryInfo>> {
        return try {
            ...
            val galleries = galleryElements.mapNotNull { element ->
    ...
    override suspend fun getPostIds(
        ...
        return try {
            ...
            val postIds = postElements.mapNotNull { element ->
    ...
    private suspend fun deletePost(
        ...
        return try {
            ...
            val pageHtml = pageResponse.body()?.string() ?: return CleaningResult.Failed("Empty page response")
            ...
            val response = gallogApi.deletePost(
                userId = _userId,
                referer = gallogUrl,
                fields = formData
            )
            
            val responseText = response.body()?.string() ?: return CleaningResult.Failed("Empty response")
            
            when {
                responseText.contains("\"result\":\"success\"") || responseText.contains("success") -> 
                    CleaningResult.Success
                responseText.contains("captcha") -> 
                    CleaningResult.CaptchaRequired
                else -> 
                    CleaningResult.Failed(responseText)
            }
        } catch (e: Exception) {
