...
@HiltViewModel
class DashboardViewModel @Inject constructor(private val repository: DcRepository) : ViewModel() {
    ...
    private fun loadGalleries() {
        viewModelScope.launch {
            ...
            repository
                    .getGalleries(uiState.postType)
                    .onSuccess { galleries ->
                        uiState = uiState.copy(galleries = galleries, isLoadingGalleries = false)
                    }
                    .onFailure { e ->
                        uiState =
                                uiState.copy(
    ...
    fun startCleaning() {
        ...
        val captchaConfig =
                if (uiState.captchaKey.isNotBlank()) {
        ...
        cleaningJob =
                viewModelScope.launch {
                    ...
                    repository
                            ...
                            .catch { e ->
                                uiState =
                                        uiState.copy(isRunning = false, error = "오류: ${e.message}")
                            }
                            .collect { progress ->
                                uiState = uiState.copy(progress = progress)

                                // Check if captcha blocked (no API key or solving failed)
                                if (progress.captchaState == CaptchaState.REQUIRED ||
                                                progress.captchaState == CaptchaState.FAILED
                                ) {
    ...
    fun stopCleaning() {
        cleaningJob?.cancel()
        cleaningJob = null
        uiState = uiState.copy(isRunning = false, progress = uiState.progress.copy(message = "중지됨"))
    }
